...
function draw(){
 ...
 //spawning boss and bossBullet
 if(score === 50){
	boss();
	bossBullet();
}

//destroying safety line
if(s <= 0){
	safetyLine.destroy();
	s=0;
}
...
//destroying bullet when touches player from aliens
 if(bulletGroup.isTouching(player)){
	 h=h-5;
	 bulletGroup.destroyEach();
 }

 //destroying aliens when touching safety line
 if(aliensGroup.isTouching(safetyLine)){
	 aliensGroup.destroyEach();
	 s=s-10;
 }

 //deestroying bullets when touching safety line
 if(bulletGroup.isTouching(safetyLine)){
	 bulletGroup.destroyEach();
	 s=s-5;
 }

 //destroying aliens by the bullet of player
...
//if both bullet touches each other both should be destroyed
if(bulletGroup.isTouching(bulletsGroup)){
	bulletGroup.destroyEach();
	bulletsGroup.destroyEach();
}

//destroying bullets when touching the boss and increasing boss
if(bulletsGroup.isTouching(bossGroup)){
	c = c+1
    bulletsGroup.destroyEach();
}

//creating sheild for player when touching the element
 if(player.isTouching(sEgroup)){
	 inviCirc.visible = true;
 }

 //increasing fuel when touching fuel element
 if(player.isTouching(fEgroup)){
	 f=f+300
	 fEgroup.destroyEach();
 }

 //increasing health when touching the health element
 if(player.isTouching(hEgroup)){
	h=h+100
	hEgroup.destroyEach();
}

// shooting bullet from pressing space
 ...
 //destroying aliens by the sheild by the power of the element
